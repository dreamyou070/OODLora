...
def main(args):
    ...
    customizing_alphas_cumprod_dict[100] = 0.40708696842193604
    customizing_alphas_cumprod_dict[120] = 0.3842085599899292
    ...
    for j, train_img in enumerate(train_images):
        if j == 0 :
            ...
            for i, present_t in enumerate(flip_times[:-1]):
                ...
                np_img = latent2image(latent , vae, return_type='np')
                pil_img = Image.fromarray(np_img)
                pil_img.save(os.path.join(output_dir, f'noising_{int(next_t.item())}.png'))  # 999
