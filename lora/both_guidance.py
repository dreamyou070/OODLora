        ...
        def forward(hidden_states, context=None, trg_indexs_list=None, mask=None):
            ...
            if is_cross_attention and mask is not None:
                if layer_name in mask.keys() :
                    mask = mask[layer_name].unsqueeze(-1) # head, pix_num, 1
...
def main(args) :
    ...
    args.output_dir = os.path.join(parent, f'recon_infer/start_random_noise_ddim_step_{args.num_ddim_steps}_'
                                           f'self_cross_map_res_{args.cross_map_res[0]}_'
            ...
            for class_name in classes:
                ...
                for j, test_image in enumerate(test_images):
                    ...
                    with torch.no_grad():
                        ...
                        time_steps = []
                        org_self_key_dict, org_self_value_dict = {}, {}
                        for i, t in enumerate(inf_time[:-1]): # 0 ~ 980
                            time_steps.append(t)
                            ...
                            for layer_name in self_key_states:
                                ...
                                if t not in org_self_key_dict.keys():
                                    org_self_key_dict[t] = {}
                                    org_self_value_dict[t] = {}
                                org_self_key_dict[t][layer_name] = key
                                org_self_value_dict[t][layer_name] = value
                        ...
                        for layer_name in self_key_states:
                            ...
                            if inf_time[-1] not in org_self_key_dict.keys():
                                ...
                                org_self_value_dict[inf_time[-1]] = {}
                            org_self_key_dict[inf_time[-1]][layer_name] = key
                            org_self_value_dict[inf_time[-1]][layer_name] = value
