                ...
                for step, batch in enumerate(train_dataloader):
                    ...
                    with accelerator.accumulate(network):
                        ...
                        with torch.set_grad_enabled(train_text_encoder):
                            if args.weighted_captions:
                                ...
                                text_encoder_conds = self.get_text_cond(args,
                                                                        ...
                                                                        text_encoders,
                                                                        weight_dtype)
                                null_text_cond = self.get_text_cond_null(args,
                                                                        accelerator,
                                                                        batch,
                                                                        tokenizers,
                                                                        text_encoders,
                        ...
                        with accelerator.autocast():
                            # -----------------------------------------------------------------------------------------------------------------------
                            noise_pred_p = self.call_unet(args,accelerator,unet,noisy_latents,timesteps,text_encoder_conds,
                                                        batch,weight_dtype,batch["trg_indexs_list"],batch['mask_imgs'])

                            noise_pred_null = self.call_unet(args, accelerator, unet, noisy_latents, timesteps,null_text_cond,
                                                             batch, weight_dtype, batch["trg_indexs_list"],batch['mask_imgs'])
