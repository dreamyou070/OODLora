                ...
                for ii, final_time in enumerate(flip_times[1:]):
                    ...
                    if final_time.item() == args.final_time:
                        ...
                        if args.use_binary_mask :
                            ...
                            start_latent = scheduler.add_noise(original_samples = st_latent, noise =randn_tensor(original_latent.shape,generator=generator, device=device, dtype=weight_dtype), timesteps = time_steps[-1])
                        else :
