...
def add_training_arguments(parser: argparse.ArgumentParser,
    ...
    parser.add_argument("--resume", action = 'store_true', default=None, help="saved state to resume training / 学習再開するモデルのstate")
...
def get_noise_noisy_latents_and_timesteps_customizing(args, noise_scheduler, latents, noise = None):
    ...
    timesteps = torch.randint(min_timestep, max_timestep, (b_size,), device=latents.device) * step_ratio
    timesteps = timesteps.long()
    # Add noise to the latents according to the noise magnitude at each timestep
    # (this is the forward diffusion process)
    if args.ip_noise_gamma:
