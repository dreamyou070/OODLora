...
class NetworkTrainer:
    ...
    def call_unet(self,unet,noisy_latents, timesteps,
                  text_conds,
                  trg_indexs_list,
                  mask_imgs):
    ...
    def train(self, args):
        ...
        if train_unet and train_text_encoder:
            if len(text_encoders) > 1:
                unet, t_enc1, t_enc2, network, optimizer, train_dataloader, lr_scheduler,student = accelerator.prepare(
                    unet, text_encoders[0], text_encoders[1], network, optimizer, train_dataloader, lr_scheduler,student
                )
            ...
            if len(text_encoders) > 1:
                t_enc1, t_enc2, network, optimizer, train_dataloader, lr_scheduler,student = accelerator.prepare(
                    text_encoders[0], text_encoders[1], network, optimizer, train_dataloader, lr_scheduler,student
                )
                text_encoder = text_encoders = [t_enc1, t_enc2]
                del t_enc1, t_enc2
            else:
                text_encoder, network, optimizer, train_dataloader, lr_scheduler,student = accelerator.prepare(
                    text_encoder, network, optimizer, train_dataloader, lr_scheduler,student
                )
                text_encoders = [text_encoder]
            ...
            for step, batch in enumerate(train_dataloader):
                ...
                with accelerator.accumulate(network):
                    ...
                    with torch.no_grad():
                        ...
                        # recon ------------------------------------------------------------------------------
                        with torch.no_grad():
                            model_output = self.call_unet(unet, noisy_latents, timesteps, text_conds,None, None)
                            model_output_masked = self.call_unet(unet, noisy_good_latents, timesteps, text_conds, None, None)


                        #noise_scheduler.step(model_output = ,
                        #                     timestep: int,
                        ...
                        train_latents = noisy_latents[train_indexs, :, :, :]
                        train_good_latents = noisy_good_latents[train_indexs, :, :, :]
                        test_latents = noisy_latents[test_indexs, :, :, :]
                        test_good_latents = noisy_good_latents[test_indexs, :, :, :]
