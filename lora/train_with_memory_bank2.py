    ...
    def train(self, args):
        ...
        class Mahalanobis_dataset(torch.utils.data.Dataset):
            ...
            def __getitem__(self, idx):
                ...
                sample['rgb_dir'] = rgb_dir
                return sample

        print(f' [1] make SAM model')
        from data.segment_anything.predictor import SamPredictor
        from data.segment_anything.build_sam import sam_model_registry
        model_type = "vit_h"
        path_to_checkpoint = r'/home/dreamyou070/pretrained_stable_diffusion/sam_vit_h_4b8939.pth'
        sam = sam_model_registry[model_type](checkpoint=path_to_checkpoint) # sam model
        ...
        for i in range(num_samples):
            ...
            rgb_dir = sample['rgb_dir']
            pil_img = Image.open(rgb_dir)
            np_img = np.array(pil_img)
            predictor.set_image(np_img)
            features = predictor.features
            ...
            with torch.no_grad():
                ...
                query_1 = controller.query_dict['up_blocks_3_attentions_1_transformer_blocks_0_attn2'][0].squeeze()  # pix_num, dim
                query_2 = controller.query_dict['up_blocks_3_attentions_2_transformer_blocks_0_attn2'][0].squeeze()  # pix_num, dim
        ...
        # pca.fit(normal_vector_np)
        # reduced_normal_np = pca.transform(normal_vector_np)
        ...
        print(f'max mahalanobis distance : {max_dist}')
        # for back_vector in back_vectors:
        #    dist = mahalanobis(back_vector.cpu(), normal_mean, normal_cov)
        #    print(f'back mahalanobis distance to normal dist : {dist}')
        """
        """
        # -------------------------------------------------------------------------------------------------------- #
